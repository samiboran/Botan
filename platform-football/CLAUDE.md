...
`Botan/` reposunun **içinde bir alt klasördür** (ayrı repo açma denendi ama
GitHub App'in repo oluşturma yetkisi olmadığı için Botan'ın altına taşındı).
Botan'daki DragonKick projesine (repo kökü) dokunma, deploy'lar da ayrı
alt yollara gider — bkz. bölüm 11.
...
- Bu proje Botan reposunun bir alt klasörü, DragonKick de aynı repoda kökte
  duruyor. İkisi de aynı GitHub Pages sitesine deploy oluyor, o yüzden
  **tek bir workflow** (`Botan/.github/workflows/deploy.yml`) her ikisini de
  build edip birleşik bir `dist/` altında yayınlıyor: DragonKick köke,
  Platform Football `dist/platform-football/` altına.
- Vite `base` ayarı bu yüzden `/Botan/platform-football/` olmalı (repo adı +
  alt klasör) — yanlışsa Pages'te boş sayfa gelir, deploy sonrası linki
  mutlaka açıp kontrol et.
- Bu klasörün kendi `.github/workflows/` dosyası **olmamalı** — sadece repo
  kökündeki workflow çalışır, ikinci bir workflow burada sessizce hiç
  tetiklenmez ve kafa karıştırır.
